    ...
    script: {
      ...
      value: config.script !== undefined ? require('path').resolve(config.script) : null
		...
		root: {
			...
			writable: true,
			configurable: false,
			value: config.runAsAgent ? `${config.logOnAsUser ? require('os').homedir() : ''}/Library/LaunchAgents` : '/Library/LaunchDaemons'
		},
		...
		logpath: {
			...
			writable: true,
			configurable: false,
			value: config.logpath || `${config.logOnAsUser ? require('os').homedir() : ''}/Library/Logs/${(this.name || config.name || 'node-scripts')}`
		},
