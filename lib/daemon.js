...
var daemon = function(config) {
	...
	config.runAsUser = config.hasOwnProperty('runAsUser') ? config.runAsUser: false;
    ...
    script: {
      ...
      value: config.script !== undefined ? require('path').resolve(config.script) : null
		...
		root: {
			...
			writable: true,
			configurable: false,
			value: config.runAsAgent ? `${config.runAsUser ? require('os').homedir() : ''}/Library/LaunchAgents` : '/Library/LaunchDaemons'
		},
		...
		logpath: {
			...
			writable: true,
			configurable: false,
			value: config.logpath || `${config.runAsUser ? require('os').homedir() : ''}/Library/Logs/${(this.name || config.name || 'node-scripts')}`
		},
