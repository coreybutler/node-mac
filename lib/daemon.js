    	...
    	get: function(){
    		...
    		if (Object.prototype.toString.call(this._ev) === '[object Array]'){
    			this._ev.forEach(function(item){
    				tmp = {};
    				tmp[this._ev[i].name] = this._ev[i].value;
    				ev.push(tmp);
    			});
    		} else {
    ...
    script: {
      ...
      value: config.script !== undefined ? require('path').resolve(config.script) : null
		...
		logpath: {
			...
			writable: true,
			configurable: false,
			value: config.logpath || p.join(process.env.HOME,'/Library/Logs/'+(this.name || config.name || 'node-scripts'))
		},
				...
				this.stop(function(){
					exec('rm '+me.plist,{},function(){
						...
						if (fs.existsSync(me.logpath)){
							if (me.logpath !== p.join(process.env.HOME,'/Library/Logs')){
